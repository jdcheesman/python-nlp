...
class Googling:
    ...
    location_finder = re.compile('<LOCATION>([^<]*)</LOCATION>')
    US_CITIES = set(gazetteers.words('uscities.txt'))
    COUNTRIES = set([country for filename in ('isocountries.txt','countries.txt')
                 for country in gazetteers.words(filename)])

    US_STATES = set(gazetteers.words('usstates.txt'))
    ...
    def guessLocation(self, data):
        ...
        for d in data:
            arr = []
            arr.extend(self.location_finder.findall(d.title))
            arr.extend(self.location_finder.findall(d.snip))
            #arr.extend(d.link.strip().split('/'))
            for w in arr:
                cnt = 1
                if w in unknown:
                    cnt = unknown[w] + 1
        ...
        sorted_data = sorted(unknown, key=unknown.__getitem__, reverse=True)
        #print sorted_data

        # first check for US states:
        state = ''
        for location in sorted_data:
            found = False
            if not found and location in self.US_STATES:
                state = location
                found = True
        ...
        found = False
        for location in sorted_data:
            if not found and location in self.COUNTRIES:
                country = location
        ...
        found = False
        for location in sorted_data:
            if not found and location in self.US_CITIES:
                city = location
                found = True


        if country == 'America' or country == 'United States':
            country = ''

        if country != '' and state != '':
            # fight!
            if unknown[state] > unknown[country]:
                print 'state>country - %s vs %s' % (state, country)
                country = '' # ignore it
            else:
                print 'country>state - %s vs %s' % (country, state)
                state = ''
        ...
        # uscity = ''
        # if len(us_cities.keys()) > 0:
        #     uscity = sorted(us_cities, key=us_cities.__getitem__, reverse=True)[0]
        # usstate = ''
        # if len(us_states.keys()) > 0:
        #     usstate = sorted(us_states, key=us_states.__getitem__, reverse=True)[0]
        # city = ''
        # if len(unknown.keys()) > 0:
        #     city = sorted(unknown, key=unknown.__getitem__, reverse=True)[0]
        # country = ''
        # if len(countries.keys()) > 0:
        #     country = sorted(countries, key=countries.__getitem__, reverse=True)[0]


        # if len(us_states.keys()) >= len(countries.keys()) or country == 'America':
        #     if us_cities[uscity] > unknown[city]:
        #         return Location(uscity, usstate)
        #     return Location(city, usstate)
